...
const submitRegistration = async function (newName, newEmail) {
  ...
  const docRef = await addDoc(collection(firestore, "aiapie-newsletter-ita"), {
    name: newName,
    email: newEmail,
  });
...
function hideForm() {
  ...
  document.getElementById("newsletter-title").innerHTML = "Congratulazioni!";
  document.getElementById("newsletter-description").innerHTML =
    "Presto riceverà un email con i primi 3 strumenti di automatizzazione da provare gratuitamente.";
}
